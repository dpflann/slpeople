...
func TestFindPossibleDuplicates(t *testing.T) {
	duplicateTestData := []struct {
	...
	}{
		{
			...
			expected: [][]string{
				{"dan@test.com", "dann@test.com", "and@test.com"},
				{"dann@test.com", "and@test.com"},
			},
		},
	}
	for _, td := range duplicateTestData {
		if result := FindPossibleDuplicates(td.strings); !cmp.Equal(result, td.expected) {
	...
	result := FindPossibleDuplicates(slEmailAddresses)
